...
class OrderManager {
    ...
    private function awardPoints(int $customerId, float $total): int {
        $points = (int)($total * POINTS_PER_TOMAN); // Calculate points based on total
    ...
    public function registerOrder(array $data, array $items): array {
        try {
            $requiredFields = [
            ...
            if (!$this->validateDate($data['order_date'])) {
                throw new Exception('تاریخ سفارش نامعتبر است یا نمی‌تواند قبل از امروز باشد.');
            }
            ...
            if (empty($productIds)) {
                throw new Exception('محصولات انتخاب‌شده نامعتبر هستند.');
            }
            ...
            foreach ($items as $index => $item) {
                if (!isset($item['product_id']) || !isset($item['quantity']) || !is_numeric($item['quantity'])) {
                    throw new Exception("داده‌های محصول در ردیف " . ($index + 1) . " نامعتبر است.");
                }
                ...
                if ($price <= 0) {
                    throw new Exception("قیمت محصول با ID $productId صفر یا نامعتبر است.");
                }
            ...
            $sql = "INSERT INTO photonim_db.`order` (
                customer_id, subtotal, description, delivery_date, discount, payment, total, balance, order_date, status, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
            ...
            foreach ($items as $item) {
                ...
                $itemStmt->execute([$orderId, $item['product_id'], $item['quantity'], $price, $subtotalItem]);
            }
            ...
            return [
                'success' => true,
                'message' => "سفارش با کد $orderId ثبت شد!" . $pointsMessage,
                'order_id' => $orderId,
                'total' => $total
            ];
        } catch (Exception $e) {
            ...
            $this->logError("Order Registration Error: " . $e->getMessage(), ['data' => $data, 'items' => $items]);
...
$newCustomerId = filter_input(INPUT_GET, 'new_customer_id', FILTER_VALIDATE_INT) ?: null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['insert'])) {
    ...
    if (isset($_POST['products']) && is_array($_POST['products'])) {
        foreach ($_POST['products'] as $index => $product) {
            if (!isset($product['id']) || !is_numeric($product['id']) || 
                !isset($product['quantity']) || !is_numeric($product['quantity'])) {
    ...
    if (!isset($errorMessage)) {
        $response = $orderManager->registerOrder($data, $items);
        if ($response['success']) {
...
<head>
    ...
    <title>استودیو نیما | ثبت سفارش</title>
...
<body>
    ...
    <div class="container">
        ...
        <section class="form-section fade-in">
            <form method="POST" id="orderForm" class="form-body">
                <div class="form-group">
                    <label class="form-label" for="customer_search"><i class="fas fa-user"></i> انتخاب مشتری *</label>
                    <div class="autocomplete-container">
                        <input type="text" id="customer_search" class="form-input" placeholder="جستجو (نام، شماره یا ID)" required <?php echo $newCustomerId ? 'disabled' : ''; ?>>
                        <input type="hidden" name="customer_id" id="customer_id" value="<?php echo htmlspecialchars($newCustomerId ?? ''); ?>" required>
                ...
                <div class="form-group">
                    ...
                    <div class="products-container">
                        <table class="order-items-table" id="products_table">
                            ...
                            <tfoot id="products_totals" style="display: none;">
                                <tr>
                                    <td colspan="5">جمع کل</td>
                                    <td id="total_quantity">0</td>
                                    <td id="total_subtotal">0</td>
                                    <td></td>
                                </tr>
                            </tfoot>
                        </table>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="delivery_date" id="delivery_date" class="form-input" placeholder="مثال: 1403-12-15" required>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="order_date" id="order_date" class="form-input" required min="<?php echo date('Y-m-d'); ?>" value="<?php echo date('Y-m-d'); ?>">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" class="form-textarea" placeholder="توضیحات سفارش (اختیاری)"></textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="payment" id="payment" class="form-input" required min="0" step="0.01">
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-select" required>
                        <?php foreach (ORDER_STATUSES as $value => $label): ?>
                            <option value="<?php echo $value; ?>" <?php echo $value === 'pending' ? 'selected' : ''; ?>>
                                <?php echo $label; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="summary-box">
                    <p><span>جمع محصولات:</span> <span id="subtotal">0</span> تومان</p>
                    <p><span>تخفیف:</span> <span id="discount_display">0</span> تومان</p>
                    <p><span>مبلغ قابل پرداخت:</span> <span id="total_display">0</span> تومان</p>
                    <p><span>پرداختی:</span> <span id="payment_display">0</span> تومان</p>
                    <p><span id="balance_label">مانده:</span> <span id="balance_display">0</span> تومان</p>
                </div>

                <button type="submit" name="insert" class="submit-btn"><i class="fas fa-check"></i> ثبت سفارش</button>
            </form>
                ...
                <tbody id="modal_product_list">
                    <?php foreach ($products as $product): ?>
                        <tr data-product-id="<?php echo htmlspecialchars($product['id']); ?>">
                            ...
                            <td><?php echo htmlspecialchars($product['imagesize'] ?? '-'); ?></td>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            <?php if ($newCustomerId): ?>
                fetch(window.location.href, {
                    ...
                    body: `action=get_customer&id=${<?php echo $newCustomerId; ?>}`
                })
                .then(response => response.json())
                .then(data => {
                    if (data.data) {
                        customerSearch.value = data.data.display;
            ...
            form.addEventListener('submit', (e) => {
                ...
                if (!orderDate.value || new Date(orderDate.value) < new Date().setHours(0, 0, 0, 0)) {
                    errors.order_date = 'تاریخ سفارش نامعتبر است یا نمی‌تواند قبل از امروز باشد';
                    isValid = false;
                }
